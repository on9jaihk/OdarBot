...
var debug = require('debug')('bot-emoji')

/**
 * regex to parse emoji in a string - finds emoji, e.g. [鼓掌]
 */
var parser = /\[([\u4e00-\u9fa5_\-\+]+)\]/g;
...
Emoji.emojify = function emojify(str, on_missing) {
  ...
  return str.split(parser) // parse emoji via regex
            .map(function parseEmoji(s, i) {
              debug('parser s %s, i %s.', s, i)
              // every second element is an emoji, e.g. "test :fast_forward:" -> [ "test ", "fast_forward" ]
              if (i % 2 === 0) return s;
              var emoji = Emoji._get(s);
              debug('_get(s) %s', emoji)
